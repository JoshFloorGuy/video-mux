...
Entry* MediaTable::deleteItem(Entry* item) {
...
Entry* MediaTable::deleteItem(string key) {
	return deleteItem(search(key));
}
...
MediaServer::MediaServer(string outputAddress) {
	...
	GstElement* mux, * sink, * scale, * scalefilter, * convertfilter, * postConvert;
	// queues
	GstElement* q1, * q2, * q3;
	GstCaps* scaleCaps;
	...
	scaleCaps = gst_caps_from_string("video/x-raw, profile=baseline, width=1280, height=720");
	...
	//videoscale
	//video / x - raw, profile = baseline, width = 160, height = 90
	//x264enc
	//flvmux
	//rtmp2sink location = rtmp://localhost/final/live

	gst_bin_add_many(GST_BIN(pipeline), switchPad, scale, scalefilter, postConvert, mux, sink, q1, q2, q3, NULL);
	if (!gst_element_link_many(switchPad, scale, scalefilter, q1, postConvert, q2, mux, q3, sink, NULL)) {
		failed = true;
...
bool MediaServer::addConnection(string incomingName) {
	...
	gst_bin_add_many(GST_BIN(pipeline), source, demux, decode, superQueue, NULL);
	gst_element_link_many(source, superQueue, demux, NULL);
	...
	if (gst_element_set_state(superQueue, GST_STATE_PLAYING) == GST_STATE_CHANGE_FAILURE) {
		g_printerr("queue2 could not start\n");
	}
...
bool MediaServer::switchConnection(string key) {
	...
	if (newSource->isActive()) return false;
	try {
