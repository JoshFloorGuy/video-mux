	...
	try
	{
		...
		while(!(*endListener))
		{
			...
			switch (str2int((*i).c_str())) {
				case ADD_STREAM:
					i++;
					cout << "Adding stream " << *i << endl;
					cout << "Stream " << *i << ((server->addConnection(*i)) ? " added successfully" : " not added") << endl;
					break;
				case SWITCH_STREAM:
					i++;
					cout << "Switching to stream " << *i << endl;
					cout << "Switch to " << *i << ((server->switchConnection(*i)) ? " was successful" : " was not successful") << endl;
					break;
				case CLOSE_STREAM:
					i++;
					cout << "Closing Stream " << *i << endl;
					cout << "Stream " << *i << ((server->closeConnection(*i)) ? " closed successfully" : " was not closed") << endl;
					break;
				default:
					break;
			}
