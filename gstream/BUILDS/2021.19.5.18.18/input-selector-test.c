...
switch_timer (GstElement * video_switch)
{
  ...
  else if (strcmp(active_name, "sink_1") == 0) {
...
{
  GstElement *pipeline, *src1, *src2, *src3, *src3filter, *video_switch, *video_sink, *segment;
  GstElement *sink1_sync, *sink2_sync, *sink3_sync, *capsfilter, *demux;
  GstBus *bus;
  ...
  src3 = gst_element_factory_make ("rtmpsrc", "src3");
  g_object_set (G_OBJECT (src3), "location", "rtmp://192.168.1.19/live/a", NULL);
  src3filter = gst_element_factory_make ("capsfilter", "caps1");
  g_object_set (G_OBJECT (src3filter), "caps",
      gst_caps_from_string ("video/x-raw,width=640,height=360"), NULL);
  demux = gst_element_factory_make("flvdemux", "demux0");
  ...
  gst_bin_add_many (GST_BIN (pipeline), src1, src2, src3, demux, segment, video_switch,
  ...
  gst_element_link (src3, demux);
  gst_element_link (demux, src3filter);
  gst_element_link (src3filter, sink3_sync);
